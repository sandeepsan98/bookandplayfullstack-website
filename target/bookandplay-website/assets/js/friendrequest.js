...
//console.log(filterPlayer);
 ...
 axios.get(url)
   ...
   axios.get(getBaseUrlFromCurrentPage()+`/GetFriendRequestList?receiverId=${userId}`)
  .then(function (response) {
	  //console.log(response.data)  
let filterData = [];
...
function playerData(array = []) {
  ...
  array.forEach((item) => {
    ...
    button_accept.setAttribute("value", item.requestId);
    ...
    button_decline.setAttribute("value", item.requestId);
...
const url = getBaseUrlFromCurrentPage()+"/AcceptFriendRequest?"; 
